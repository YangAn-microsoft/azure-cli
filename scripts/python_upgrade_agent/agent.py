...
Usage:
    python -m scripts.python_upgrade_agent.agent
    python -m scripts.python_upgrade_agent.agent --dry-run
    python -m scripts.python_upgrade_agent.agent \\
...
def main(argv: list[str] | None = None) -> int:
    ...
    # --verbose is implied by --dry-run; --log-file implies --verbose.
    verbose_enabled = args.verbose or args.dry_run or args.log_file is not None
    if verbose_enabled:
        log_path = args.log_file or (repo_root / "python_upgrade_agent.log")
    else:
        log_path = None
    vlog = _VerboseLog(log_path)
    if vlog.enabled:
        print(f"agent: verbose log -> {log_path}")
        vlog.write(f"argv: {argv if argv is not None else sys.argv[1:]}")
        vlog.write(f"repo_root: {repo_root}")
        vlog.write(f"repo: {repo or '(unset)'}")
        vlog.write(f"model: {args.model}")
        vlog.write(f"dry_run: {args.dry_run}")

    # --- Step 1: detect ---
    detected_current = detect.read_current_version(repo_root)
    detected_target: detect.Version | None = None
    if not (args.force_new_minor or args.force_new_patch):
        try:
            releases = detect.fetch_python_releases()
        except Exception as exc:  # noqa: BLE001
            print(f"agent: python.org feed unreachable: {exc}", file=sys.stderr)
            return 0  # exit clean per failure handling policy
    ...
    current, target = _apply_force_overrides(detected_current, detected_target, args)
    ...
    ref_repo = args.reference_repo or repo
    if ref_repo:
        exclude = set(args.exclude_pr)
    ...
    for attempt in (1, 2):
        ...
        try:
            ...
            raw = ai.call_model(ai.SYSTEM_PROMPT, user_msg, model=args.model)
    ...
    github_ops.create_branch(repo_root, branch, base=args.base_branch)
    ...
    number = github_ops.open_draft_pr(
        repo=repo, title=title, body=body, head=branch, base=args.base_branch,
    )
